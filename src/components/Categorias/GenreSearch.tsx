...
export default function GenreSearch({ searchTerm, onSearchChange, resultsCount }: GenreSearchProps) {
  return (
    <div className="max-w-md relative" style={{ zIndex: 100 }}>
      ...
      {searchTerm && resultsCount !== undefined && (
        <p className="text-sm mt-2" style={{ color: 'rgba(103, 89, 78, 0.7)' }}>
          {resultsCount} {resultsCount === 1 ? 'categoria encontrada' : 'categorias encontradas'}
        </p>
      )}
    </div>
  );
